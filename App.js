...
    // Regex for common special characters: !@#$%^&*()_+{}[]:;<>,.?~\\-=/
    // Reverted to original, correct regex syntax.
...
  // Determine the color for the strength indicator based on the score (background color)
...
  };
  ...
  // Determine the color for the strength text based on the score (text color)
  const getStrengthTextColor = (score) => {
    if (score === 5) return 'text-green-600'; // Very Strong
    if (score >= 4) return 'text-lime-600';   // Strong (using 600 for better contrast with light background)
    if (score >= 3) return 'text-yellow-600'; // Moderate
    if (score >= 1) return 'text-orange-600'; // Weak
    return 'text-red-600'; // Very Weak
  };
...
            ></div>
          </div>
          <p className={`text-center text-sm font-medium ${getStrengthTextColor(strength)}`}>
